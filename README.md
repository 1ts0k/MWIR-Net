...
结构分支公平消融批量运行（3 个 seed，8 epoch）：

```bash
python tools/run_fair_ablation_suite.py \
  --cuda 0 \
  --epochs 8 \
  --seeds 0 1 2 \
  --modes zero_prompt no_channel_attention \
  --run-id fair_ablation_3seed_e8_root \
  --ckpt-root /root/mwirnet_runs/checkpoints \
  --output-root /root/mwirnet_runs/outputs \
  --log-root /root/mwirnet_runs/logs
```
...
统计日期：2026-04-28；公平消融实验补充于 2026-05-07。下表摘自当前工作区保存图像的统一复算结果。PSNR、SSIM 越高越好，LPIPS(alex) 越低越好。
...
Rain100L 结构分支公平消融采用 `MWIR-Net-5k_12epoch_init` 权重初始化，去雨单任务训练 8 epoch，并在 seed 0/1/2 上重复。当前 3-seed 结果如下：

| 消融模式 | PSNR mean±std | SSIM mean±std | LPIPS(alex) mean±std |
|---|---:|---:|---:|
...
两组均值差异小于跨 seed 标准差：`no_channel_attention` 的 PSNR 高 0.0089 dB，SSIM 低 0.000008，LPIPS 低 0.000223。该结果说明在当前 8 epoch 去雨微调协议下，去除 prompt 通道注意力没有形成稳定劣化；早期 2 epoch 单次消融中“通道注意力明显有益”的判断应改为趋势性参考。

## 可视化对比

对比图原图保存在 `visual_comparisons/rain100l_comparison.png` 和 `visual_comparisons/sots_outdoor_comparison.png`。PPT 使用的对比图保存在 `visual_comparisons/ppt_5k_protocol/`，其中保留了 Rain100L 和 SOTS outdoor 的网格图与单样本横向对比条。额外的 Rain100L 消融、Rain100H、Test1200、Test2800 和 NYU-Haze500 网格图放在 `visual_comparisons/extra_benchmarks/`。少量论文会直接引用的原始输入/清晰参考图已复制到 `visual_comparisons/original_samples/`，论文样本包则集中在 `visual_comparisons/thesis_sample_pack/`，把原 pair 和各模型效果图放在一起，便于直接挑图和写正文。
