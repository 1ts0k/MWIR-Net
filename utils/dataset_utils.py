...
class MWIRTrainDataset(Dataset):
    def __init__(self, args):
        super(MWIRTrainDataset, self).__init__()
        self.args = args
        self.rs_ids = []
        self.hazy_ids = []
    ...
    def _init_hazy_ids(self):
        ...
        self.hazy_ids = self._limit_ids(self.hazy_ids, "max_dehaze", "Hazy")
    ...
    def _init_rs_ids(self):
        ...
        repeat = max(1, int(getattr(self.args, "derain_repeat", 1)))
        self.rs_ids = self.rs_ids * repeat
        self.rs_ids = self._limit_ids(self.rs_ids, "max_derain", "Rainy")

        self.rl_counter = 0
        self.num_rl = len(self.rs_ids)
        print("Total Rainy Ids : {}".format(self.num_rl))

    def _limit_ids(self, ids, arg_name, label):
        max_count = int(getattr(self.args, arg_name, 0))
        if max_count <= 0 or len(ids) <= max_count:
            return ids

        seed = int(getattr(self.args, "subset_seed", 0))
        task_offset = {"Rainy": self.de_dict["derain"], "Hazy": self.de_dict["dehaze"]}.get(label, 0)
        rng = random.Random(seed + task_offset)
        sampled_ids = rng.sample(ids, max_count)
        print("Using {} {} Ids from {} with subset_seed {}".format(
            max_count, label, len(ids), seed
        ))
        return sampled_ids
...
class DerainDehazeDataset(Dataset):
    ...
    def _match_pair_size(self, degraded_img, clean_img, base=16):
        height = min(degraded_img.shape[0], clean_img.shape[0])
        width = min(degraded_img.shape[1], clean_img.shape[1])
        height = height - height % base
        width = width - width % base

        def center_crop(image):
            top = max(0, (image.shape[0] - height) // 2)
            left = max(0, (image.shape[1] - width) // 2)
            return image[top:top + height, left:left + width]

        return center_crop(degraded_img), center_crop(clean_img)
    ...
    def __getitem__(self, idx):
        ...
        if degraded_img.shape != clean_img.shape:
            degraded_img, clean_img = self._match_pair_size(degraded_img, clean_img, base=16)
