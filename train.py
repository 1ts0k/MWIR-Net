...
def main():
    ...
    seed = int(getattr(opt, "seed", -1))
    if seed >= 0:
        seed_everything(seed, workers=True)
        print(f"Seeded training with seed={seed}")
    ...
    if use_wandb:
        ...
        csv_log_dir = getattr(opt, "csv_log_dir", None)
        csv_log_dir = str(csv_log_dir).strip() if csv_log_dir is not None else ""
        if csv_log_dir.lower() in {"", "none", "false", "off"}:
            csv_log_dir = "logs"
        logger = CSVLogger(save_dir=csv_log_dir, name="mwirnet")
    ...
    if bool(getattr(opt, "save_last_only", False)):
