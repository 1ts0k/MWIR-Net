## MWIR-Net: Multi-scale Weather-aware Image Restoration Network
## Built for fog/rain image restoration experiments.
...
##########################################################################
##---------- Weather-aware Prompt Module -----------------------
class PromptChannelAttention(nn.Module):
    def __init__(self, channels, reduction=8, enabled=True):
        super(PromptChannelAttention, self).__init__()
...
class WeatherPromptBlock(nn.Module):
    def __init__(self,prompt_dim=128,prompt_len=5,prompt_size = 96,lin_dim = 192,ablation_mode="full"):
        super(WeatherPromptBlock,self).__init__()
        if ablation_mode not in {"full", "zero_prompt", "no_channel_attention"}:
            raise ValueError("ablation_mode must be full, zero_prompt, or no_channel_attention")
        self.prompt_dim = prompt_dim
        self.ablation_mode = ablation_mode
        self.prompt_param = nn.Parameter(torch.rand(1,prompt_len,prompt_dim,prompt_size,prompt_size))
        self.linear_layer = nn.Linear(lin_dim,prompt_len)
        self.conv3x3 = nn.Conv2d(prompt_dim,prompt_dim,kernel_size=3,stride=1,padding=1,bias=False)
        self.channel_attn = PromptChannelAttention(
            prompt_dim,
            enabled=ablation_mode != "no_channel_attention",
        )
        

    def forward(self,x):
        B,C,H,W = x.shape
        if self.ablation_mode == "zero_prompt":
            return x.new_zeros(B, self.prompt_dim, H, W)
        ...
        prompt = self.channel_attn(prompt)
...
class MWIRNet(nn.Module):
    def __init__(self, 
        ...
        if self.decoder:
            self.prompt1 = WeatherPromptBlock(prompt_dim=64,prompt_len=5,prompt_size = 64,lin_dim = 96,ablation_mode=ablation_mode)
            self.prompt2 = WeatherPromptBlock(prompt_dim=128,prompt_len=5,prompt_size = 32,lin_dim = 192,ablation_mode=ablation_mode)
            self.prompt3 = WeatherPromptBlock(prompt_dim=320,prompt_len=5,prompt_size = 16,lin_dim = 384,ablation_mode=ablation_mode)
